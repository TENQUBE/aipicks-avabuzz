...
const GlobalOptionsContext = createContext<any>({
  theme: 'android'
})
...
export const basicUIPlugin: (
  options: BasicUIPluginOptions | ((args: { initialContext?: any }) => BasicUIPluginOptions)
) => StackflowReactPlugin = (options) => () => ({
  key: 'basic-ui',
  wrapStack({ stack, initialContext }) {
    const _options = (typeof options === 'function' ? options({ initialContext }) : options) as any

    const noAnimate = initialContext.req.noAnimate
        ...
        <div
          ...
          style={assignInlineVars(
            compactMap({
              ...
              [css.globalVars.appBar.minSafeAreaInsetTop]: _options.appBar?.minSafeAreaInsetTop,
              [css.globalVars.bottomSheet.borderRadius]: _options.bottomSheet?.borderRadius,
